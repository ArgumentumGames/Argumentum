...
3. **Mind Maps** (`MindMapCreator`)
   ...
   ⚠️ **The 34 `<lang>/` wrappers split 17/17, and only one half carries anything** (measured 2026-09-20 on `c56ec6d6`, by counting `<svg` file by file). The **17 inlining** wrappers (`Fallacies_<lang>.html`, `Argumentation_Virtues_<lang>.html`, plus FR-only `Fallacies_cards_fr.html`) embed the SVG **verbatim** — 508 KB to 5.9 MB each, and they are what a reviewer opens. The **17 `_ext`** wrappers reference it through `<object data="…">` and are ~91 KB, therefore carrying **zero** URLs, zero thumbnails, zero node text *by construction* — an instrument that counts them as defect surface will read a structural zero as a clean bill. The 2 root templates likewise. ⇒ **An anti-collapse floor belongs on the inlining subset (17), never on the total (34)**: a floor of 34 is red on a healthy tree.

   ⚠️ **A re-derivation does NOT refresh the wrappers unless a flag is explicitly forced — and the skip is silent** (measured 2026-09-20, [#1438 c.5750260550](https://github.com/ArgumentumGames/Argumentum/issues/1438#issuecomment-5750260550)). `OverwriteExistingHtmlMaps` (`AssetConverterConfig.cs:460`) has **no initializer** ⇒ `false`, and the guard is written three times (`FallacyMindMapDocumentConfig.cs:1702`, `MindMapDocumentConfig.cs:772`, `VirtueMindMapDocumentConfig.cs:871`):

   ```csharp
   if (File.Exists(htmlFileName) && !config.OverwriteExistingHtmlMaps)
       Logger.Log($"Skip existing Html SVG Wrapper: {htmlFileName}");   // Log, NOT LogWarning
   ```

   Every committed wrapper exists ⇒ every one is skipped. A pass run without the named gesture repairs the SVGs and ships wrappers still inlining the **pre-repair** SVG, with only an INFO line to show for it — the same family of silent-green as the XSLT fallback below. The two `--regen-*` entry points (`Program.cs:429` for Fallacies #820, `:538` for Virtues #826) force it to `true`, but **neither is the re-derivation** — the first states in its own comment that it *"NEVER runs the normal Mindmapper pipeline"*. ⇒ On a re-derivation pass, set `OverwriteExistingHtmlMaps = true`, run in **Debug** (`TemplatePathRelease` is a `raw.githubusercontent.com/.../master/` URL — same trap as `JsonFilePathRelease`, #1225/#1228), and verify **in delta**: `grep -c "Skip existing Html SVG Wrapper"` = 0 and `grep -c "wrapper .* successfully saved"` = 34 (= **17 `content.svg` × 2 wrappers each** — the config rewrites the 17 `_ext` too, same loop: `FallacyMindMapCreatorConfig.cs:84`/`:156`, `VirtueMindMapCreatorConfig.cs:61`; this 34 is the **rewrite** surface, not the defect surface forbidden above). `MindmapWrapperFreshnessGateTests` (#1446) catches the skip *on the tree* afterwards, by flagging any inlining wrapper committed older than the `.content.svg` it embeds.

   ⚠️ Corollary for any conclusion drawn from a config default: **a default describes what is possible, not what is practised.** Reading `false` here yields "the wrappers are never rewritten" — which the artefact history refutes (`4bd53c81` touches 9 `.svg` + 3 `.html`; `f5acc7be` 27 + 9 — one inlining wrapper per language touched). Confront a default with `git show --name-only` on the commits that performed the gesture before concluding.
...
**Totaux fabrication** (#1187, décision retail du 14/09 c.`5665864605`, une boîte par langue) : deck **Tarot = 197 cartes** (175 Fallacies + 15 Rules + memo ×7 exemplaires imprimés) · deck **Scenarii = 167 cartes** · boîte **sans** Virtues = **364 cartes**, **avec** = **495**. *(Erratum 15/09 : les volumes 191/358/489 publiés le 05/09 — qui comptaient une seule memo — sont superseded pour la fabrication physique ; P&P inchangé, memo ×5/×1.)*

⚠️ **#1288 (décision owner du 05/09/2026, épinglée en CI par `PdfDeckCountContractTests`)** : PK 96 est un vrai doublon de PK 108 « Appel à la nature » et sort du deck ⇒ Fallacies 176→**175**, Tarot 192→**191**, boîte 359→**358** / 490→**489** (Scenarii 167 inchangé). Comptes re-mesurés le 15/09 sur `c089d526` depuis les CSV (Fallacies `carte` non vide = 175 PK uniques ; Rules 15 ; Scenarii 167 ; memo ×7 = décision #1187 ; Virtues 131 rapporté, arbre 22/08) ; le bundle mesuré du 11/09 les conforte (175 faces, 197 instances, 379 pages). Le compte de PNG de l'**arbre de régénération** (`bin/*/Target/`) reste **à re-mesurer** à la prochaine régén (lane po-2023). ⚠️ **En revanche, les mindmaps committés sont déjà à jour** — l'affirmation « ils embarquent encore les 176 thumbnails d'avant #1288 » est **fausse, mesurée le 19/09** : `Cards/Fallacies/Mindmaps/fr/Argumentum_Fallacies_MindMap_cards_fr.content.svg` porte **exactement 175 balises `<image>`**, toutes `width="60"`, et a été re-dérivé le 09/09 (`4bd53c81`, relabel PK 598) donc **après** la décision #1288 du 05/09. Ce sont **deux objets distincts** : l'arbre de build n'est pas le dépôt.
> 🔧 **Instrument** — les vignettes sont **encodées en base64 inline**, pas référencées par chemin : `grep -c '\.png'` rend **0** et se lit à tort comme « aucune vignette ». Compter `grep -o 'xlink:href="data:image/png;base64' <fichier> | wc -l`. Et ⛔ **ne pas compter un libellé** pour compter des cartes : le texte SVG est découpé en `tspan` au retour à la ligne, si bien qu'un libellé connu comme unique rend 1, 2 **ou** 3 occurrences (mesuré : « Faux dilemme » 1, « Appel à la nature » 2, « Homme de paille » 3). Le compte de libellés n'établit rien, **dans aucun sens**.
...
- **Models**: **`gpt-5.6-sol`** — le modèle **décidé** (GO owner 14/08) est désormais aussi celui que la **config compile** ; `gpt-4.1`/`gpt-4.1-mini` restent le fallback RU/PT (13 occurrences, inchangé). ⚠️ **L'erratum du 2026-09-15 est SUPERSEDED** — il chiffrait la config comme encore majoritairement `gpt-5.5`, sans aucune occurrence de 5.6, et donnait la migration pour gatée sur un GO à nommer (⛔ **chiffres volontairement non recopiés ici** : un remède qui **cite** le défaut le ré-indexe, et le prochain `grep` le relit comme courant — décrire, jamais citer) : la migration a été **mergée par #1426** (`0a79d217`), commentaires périmés historisés par #1431. **Re-mesuré le 2026-09-21 sur `22486d4a`** (`DatasetUpdaterRootConfig.cs`) : **48 littéraux `"gpt-5.6-sol"`, 0× `gpt-5.5`, 0× `gpt-5.4`** (87 occurrences toutes formes — les deux chiffres mesurent des objets différents, littéraux quotés vs commentaires inclus). ⛔ **Ne plus dispatcher « bumper la config » : c'est fait.** Ce qui reste gaté est **l'exécution** (GO owner + clé nommée), jamais le bump — voir [`docs/translation/202-smoke-datasetupdater-protocole-2026-09-15.md`](docs/translation/202-smoke-datasetupdater-protocole-2026-09-15.md). ⚠️ Deux `.cs` nomment encore `gpt-5.5` (`RulesSectionLexiconTests.cs:74`, `DnnUiString.cs:11`) : ce sont des **constats historiques datés** (« arbitré par… le 2026-08-28 »), pas des états courants — ⛔ ne pas les « corriger », ce serait falsifier un journal.
...
- **Re-mesure i18n `es`/`ar`/`fa`/`zh`** — mesurée 2026-09-13 sur master `35acac04`, 5 corpus (Fallacies, Virtues, Scenarii, Rules, Rules PP), 12 774 cellules par langue : **couverture 100 %** sur tous les champs substantiels (les seuls vides sont **structurels**, identiques dans les 8 langues) et **0 contamination FR**. La seule lacune apparente est `link` — ⚠️ **mais ce chiffre mesure la colonne brute, pas ce qui est livré** (re-mesuré 19/09, `ea7a0a99`, **issue #1435**). `Entities/Fallacy.cs:22-39` déclare une **cascade de repli** (`Link<Lang>Fallback` → `link_<lang>` ?? `link_en` ?? `link_fr`), et ce sont ces propriétés-là que consomme le générateur de mindmaps (`MmGeneratorTests.cs:392` asserte `LinkExpression` ⊇ `LinkFrFallback`). Sur **le deck imprimé** (175 cartes) : brut `fr` 49,7 % · `en` 90,3 % · `ru` 15,4 % · les 5 autres 4–5 % ⇒ **après repli, 161/175 = 92,0 % uniformes sur les 8 langues**, puis **165/175 = 94,3 %** depuis #1437. ⚠️ **Le tri des 14 cartes sans lien est SOLDÉ et #1435 est CLOSE** (19/09, `COMPLETED`) : #1437 a rempli **4 PK** (**2, 666, 758, 833**) et les **10 restants sont des vides JUSTIFIÉS, consignés nommément** (PK **1, 33, 658, 681, 690, 798, 826, 876, 1004, 1281**) — têtes de famille abstraites sans article dédié. ⛔ Ne pas re-dispatcher « trancher carte par carte » sur la foi de cette ligne : c'est fait. ⚠️ Les deux bases diffèrent : les pourcentages de 2026-09-13 portent sur les **1408 nœuds**, ceux-ci sur les **175 cartes** — vrais tous les deux, mais seul le second décide d'un livrable. Et `link` n'est rendu par **aucun gabarit de carte** (zéro `{{link}}` dans `Cards/**/*.json`) : le remplir n'impose **aucune régénération de PDF**. 6 groupes ont exigé une lecture ligne à ligne (tous des locutions latines, un emprunt accentué, une citation, deux formes espagnoles autonomes). Deux **questions éditoriales** étaient signalées : `Scenarii.<lang>.subcategory` « campagne » — ✅ **corrigé** par #1434 (`Campaign` rendu au sens *rural* en `es`/`ar`/`zh`/`fa`, 16 cellules) — et « romance » (`es`) vs `Romance` (`en`/`pt`), **laissée en l'état** (voisinage désambiguïsant, à ne changer que sur jugement natif). Instrument, tables et relevé nominatif : [`docs/translation/i18n-remesure-es-ar-fa-zh.md`](docs/translation/i18n-remesure-es-ar-fa-zh.md)
...
9. Valider DatasetUpdater round-trip avec OpenAI API (3-5 records, `Enabled=true`) — **protocole rédigé, jamais exécuté** ([`202-smoke-datasetupdater-protocole`](docs/translation/202-smoke-datasetupdater-protocole-2026-09-15.md)). ⚠️ « smoke test gpt-5.5 en cours (po-2023, cycle 61bis) » était **doublement périmé** : ni en cours, ni gpt-5.5. Le prérequis **P1 (bump de config) est SOLDÉ** par #1426 ; restent **P2 (clé nommée, hors dépôt)** et P3-P5 (sonde crédit, API Responses + effort low, copie fraîche du corpus) — à poser à l'exécution. Le gate est le **GO owner + la clé**, ⛔ plus le bump.
