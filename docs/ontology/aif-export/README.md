...
- **Typed edges carry distinct modes.** `undercut` (attacks the rule/inference, RA-node), `undermine` (attacks the premise, I-node), and `rebut` (a **counter-argument**, marked `CA-node`) are **three different argumentative operations**. Collapsing them into a single unweighted adjacency collapses the mode-of-attack signal — the dimension a strate-6 spectral analysis most wants to surface. Do not binarise the typed edges. *Node-type labelling follows the export's own axiom (see the verdict table above: `undercut → RA`, `undermine → I`, `rebut → CA`; `node_type_axiom_ok = 1` for all 145 rows). The `CA` marker for `rebut` denotes the **conflicting-application / counter-argument** mode and is what keeps the three attack types on three distinct node-types — note that in canonical AIF/ASPIC+ a rebuttal's semantic target is the conclusion (an I-node); here `CA` is the export's mode-marker, not the canonical target node. A consumer should read it as a **type channel**, not as an AIF node-instance.*
