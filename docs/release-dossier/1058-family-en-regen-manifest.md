...
> Note for ai-01 (corrected 2026-08-11): an earlier version of this note claimed *« le mindmap Fallacies EN affiche les familles en FR, contrairement aux 6 autres langues localisées »* and recommended a post-tag issue. **That claim is false — killed by the inverse control ai-01 prescribed in dispatch `kkieqj` before any issue was opened.** Measured on the published `Cards/Fallacies/Mindmaps/en/Fallacies_en.content.svg`: the **visible** family-node `<text>` labels are **EN** — all 8 EN families appear (`Cheating`, `Fallacy`, `Faulty logics`, `Influence`, `Insufficiency`, `Mathematical error`, `Misleading language`, `Obstruction`). The `familyclass` attribute is FR (`Tricherie`, `Influence`, …) but that is a **CSS colour key**, invisible, and FR **by design for all 8 languages** (the `card.<Famille>` CSS rules are keyed on the FR family name, so `familyclass` must be FR to match — `Fallacy.cs:15` `Family => Famille` guarantees this). The code-level reasoning above (`Family=>Famille` alias) is correct for the `familyclass` attribute but was wrongly projected onto the visible label, which is localised through a different path. **No issue opened; no defect.** (Self-correction of a claim that shipped in the first version of this manifest.)
