# -*- coding: utf-8 -*-
"""G3 (#1499) — Dossier d'arbitrage Fallacies : champ `desc_fr` par COUCHE + liste courte.

PÉRIMÈTRE (pool #458 c.5796151802, lane po-2024 v13) : dossier `desc_fr`
LIVRÉ DIRECTEMENT avec sa liste courte des pertes (format G2-C), sous la
règle C (owner 23/09 12:35Z, #1499 c.5794911968) : ⛔ aucune question de
régime — défaut = garder HEAD, la liste ne retient que ce qui mérite de
revenir (perte d'un fait/d'une nuance, erreur introduite, longueur).
...
RÉFÉRENCES (3, par ordre d'autorité décroissant)
  ...
  - `62b561e75` baseline canonique (22/04/2024) — référence des COUCHES
    ⚠️ la baseline n'est PAS l'imprimé.
  - HEAD — corpus courant.

ARCHITECTURE (motif G2 #1518, à l'identique)
  Contrôle 5 : ré-appariement des SŒURS renumérotées par TITRE, sens
  archive → HEAD uniquement. Passe 1 : imprimé (2022 > v3) → HEAD par
  `path` + contrôle 5, filtre `carte ≠ vide`. Passe 2 : PK baseline
  imprimées → HEAD. Couches par PK contre baseline sur desc_fr.
  Contrôle 6 : un SUBST dont le texte vit verbatim ailleurs est un
...
TÉMOINS À CHAQUE RUN
  ...
  (d) témoin positif CALIBRÉ par sonde préalable (23/09, ce dépôt) :
      PK 2 (path 1.1) desc_fr non-IDENT — 9/10 PK sondés en écart
      baseline→HEAD, PK 2 confirmé changé (87 c. des deux côtés).
...
FIELD = 'desc_fr'
OUT_JSON = 'fallacies-desc-diff.json'
...
def classify(ref, now):
    ...
    if skeleton(r) in skeleton(n):
        return 'AJOUT', 'la référence est contenue dans le texte actuel → ajout en place'
...
def main():
    ...
    # ---- PASSE 1 : imprimé -> HEAD (path + contrôle 5), desc_fr ----
    out = []
    seen_pk = set()
    misjoined_targets = set(misjoined.values())
    for p in sorted(H_path):
        ...
        if p in repair:
            ...
            ref_path = repair[p]
        elif p in A_path and p not in misjoined_targets:
    ...
    print('(d) témoin PK 2 (path 1.1, calibré par sonde) desc_fr non-IDENT : %s'
          % ('PASS' if d_ok else 'FAIL'))
    print('contrôle 5 : %d jointures path entre sœurs ré-appariées' % len(repair))
    print('contrôle 6 : SUBST = déplacement verbatim : %d %s' % (len(moved), sorted(moved)))

    # ---- VENTILATION ----
    from collections import Counter
    print()
    print('VENTILATION desc_fr (classes vs imprimé × couche baseline)')
    ...
    print()
    print('  TOTAL lignes desc_fr non-IDENT : %d' % len(out))
    ...
    print()
    print('-> %d lignes dans %s' % (len(out), OUT_JSON))
