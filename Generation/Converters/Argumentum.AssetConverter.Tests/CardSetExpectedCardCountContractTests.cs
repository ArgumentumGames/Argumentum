...
namespace Argumentum.AssetConverter.Tests
{
	...
	///
	/// Le Memo n'a PAS de Fact ici — inapplicabilité mesurée, pas un choix d'architecture :
	/// <c>Cards/Memo/</c> ne contient aucun CSV autonome (2 templates JSON + Archive + Assets,
	/// mesuré le 11/09 sur <c>6108194d</c>) : sa face lit la taxonomie Fallacies via le même
	/// DataSet + filtre « carte » ∈ {1,2} que Fallacies. Son compte « 1 carte » est donc épinglé
	/// au niveau dérivation, seul à voir toute la chaîne (lignes filtrées ÷ rscount du template
	/// → cartes, × NbCopies de la config → instances) :
	/// <c>PdfDeckCountContractTests.Memo_Card_Count_Is_Derived_From_Template_Rscount_Not_Hardcoded</c>
	/// (#1187, qui trace aussi la divergence « 1 carte récoltée vs ×7 copies »). L'ancien Fact
	/// skippé attendait un CSV que rien ne lit jamais ; retiré le 11/09 — son message (« le
	/// contrat Memo=1 n'est PAS épinglé depuis une source CSV ») était devenu faux depuis que
	/// l'organe niveau 2 couvre le maillon, et son invitation à ajouter
	/// <c>Cards/Memo/Argumentum_Memo.csv</c> décrivait un chemin absent de toute config.
	/// </summary>
	public class CardSetExpectedCardCountContractTests
	{
		...
		// ─────────────────────────────────────────────────────────────────────────
		// (5) L'étiquette d'encodage atteint réellement le message d'échec (#1213 / review T2.2).
